...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp)
{
    ...
    if (mState == NOT_INITIALIZED || mState == NO_IMAGES_YET)  //特征点提取等
...
bool Tracking::TrackWithIMU(bool bMapUpdated)//替换了原来的 TrackWithMotionModel();
{
    ...
    /*if (mSensor != System::STEREO)
        ...
        th = 7;*/
    int nmatches = matcher.SearchByProjection(mCurrentFrame, mLastFrame, th, mSensor == System::MONOCULAR);  //通过重投影寻找匹配
...
IMUPreintegrator Tracking::GetIMUPreIntSinceLastKF(Frame* pCurF, KeyFrame* pLastKF, const std::vector<IMUData>& vIMUSInceLastKF)
{
    ...
    // remember to consider the gap between the last KF and the first IMU
    {
        ...
        if (dt < 0)
        {
            ...
            cerr.unsetf ( ios::showbase );                // deactivate showbase
        }
    }
    // integrate each imu
    for (size_t i = 0; i < vIMUSInceLastKF.size(); i++)
    {
        ...
        if (dt <= 0)
        {
            cerr << fixed << setprecision(3) << "tracking:dt = " << dt << ", this vs next time: " << imu._t << " vs " << nextt << endl;
            cerr.unsetf ( ios::showbase );                // deactivate showbase
        }
    }
...
cv::Mat Tracking::GrabImageMonoVI(const cv::Mat &im, const std::vector<IMUData> &vimu, const double &timestamp)
{
    ...
    if (mState == NOT_INITIALIZED || mState == NO_IMAGES_YET)  //没初始化或者没有关键帧
        mCurrentFrame = Frame(mImGray, timestamp, vimu, mpIniORBextractor, mK, mDistCoef, mbf, mThDepth);
    else
        mCurrentFrame = Frame(mImGray, timestamp, vimu, mpORBextractorLeft, mK, mDistCoef, mbf, mThDepth, mpLastKeyFrame);
...
Tracking::Tracking(System *pSys,  FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer,
    ...
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap)
{
    ...
    cv::Mat DistCoef(4, 1, CV_32F); //畸变参数
    ...
    else
    {
        ...
        if (bOK)        
        {
            ...
            if (mbRelocBiasPrepare)
            {
                ...
                if (true)//mCurrentFrame.mnId == mnLastRelocFrameId + 20 - 1)
                {
                    NavState nscur;
    ...
    if (!mpInitializer)
    {
        ...
        if (mCurrentFrame.mvKeys.size() > 100)
        {
            ...
            for (size_t i = 0; i < mCurrentFrame.mvKeysUn.size(); i++)
                mvbPrevMatched[i] = mCurrentFrame.mvKeysUn[i].pt;   //参考帧特征点

            if (mpInitializer)
                delete mpInitializer;

            mpInitializer =  new Initializer(mCurrentFrame, 1.0, 200);//注册初始器
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    // Create KeyFrames
    KeyFrame* pKFini = new KeyFrame(mInitialFrame, mpMap, vimu1, NULL);
    pKFini->ComputePreInt();
    KeyFrame* pKFcur = new KeyFrame(mCurrentFrame, mpMap, vimu2, pKFini);
...
bool Tracking::TrackWithMotionModel()
{
    ...
    if(!mVelocity.empty())
      ...
      cerr<<"error!!!,mVelocity is empty"<<endl;
...
void Tracking::CreateNewKeyFrame()
{
    ...
    KeyFrame* pKF = new KeyFrame(mCurrentFrame, mpMap, mvIMUSinceLastKF, mpLastKeyFrame);
