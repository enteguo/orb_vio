...
public:
    ...
    //灰度图，时间戳，imu数据，orb特征，内参K，畸变参数，立体视觉baseline，深度阈值
    ...
    std::vector<IMUData> mvIMUDataSinceLastFrame;  //两帧之间的IMU测量值

    // For pose optimization, use as prior and prior information(inverse covariance)
    Matrix<double,15,15> mMargCovInv;
    NavState mNavStatePrior;  //前面的IMU状态

protected:
    NavState mNavState;   //当前imu状态
...
//一个flag，only for the first Frame (or after a change in the calibration
