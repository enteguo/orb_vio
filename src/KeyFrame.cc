...
KeyFrame::KeyFrame(Frame &F, Map* pMap, std::vector<IMUData> vIMUData, KeyFrame* pPrevKF):
    ...
    mnMaxY(F.mnMaxY), mK(F.mK), mvpMapPoints(F.mvpMapPoints), mbFirstConnection(true), mpParent(NULL), mbNotErase(false),
...
KeyFrame::KeyFrame(Frame &F, Map *pMap):
    ...
    mnBAGlobalForKF(0),fx(F.fx), fy(F.fy), cx(F.cx), cy(F.cy), invfx(F.invfx), invfy(F.invfy),
    mbf(F.mbf), mb(F.mb), mThDepth(F.mThDepth), N(F.N), mvKeys(F.mvKeys), mvKeysUn(F.mvKeysUn),
    mvuRight(F.mvuRight), mvDepth(F.mvDepth), mDescriptors(F.mDescriptors.clone()), mnScaleLevels(F.mnScaleLevels), mfScaleFactor(F.mfScaleFactor),mfLogScaleFactor(F.mfLogScaleFactor), mvScaleFactors(F.mvScaleFactors), mvLevelSigma2(F.mvLevelSigma2),mvInvLevelSigma2(F.mvInvLevelSigma2), mnMinX(F.mnMinX), mnMinY(F.mnMinY), mnMaxX(F.mnMaxX),
    mnMaxY(F.mnMaxY), mK(F.mK), mvpMapPoints(F.mvpMapPoints), mbFirstConnection(true), mpParent(NULL), mbNotErase(false),
    mbToBeErased(false), mbBad(false), mHalfBaseline(F.mb/2), mpMap(pMap)
{
    // Test log
...
void KeyFrame::UpdateConnections()
{
    ...
    sort(vPairs.begin(),vPairs.end());   //由于map结构没有sort函数，所以用vector
    list<KeyFrame*> lKFs;
    list<int> lWs;
