...
void loadIMUFile(char * imuPath, std::vector<ORB_SLAM2::IMUData> &vimuData)
{
    ...
    if(inf.is_open())
      cout<<"read IMU data from"<<imuPath<<endl;
    else
      cerr<<"open IMU data file fail"<<endl;
...
int main(int argc, char **argv)
{
    ...
    if (argc != 6)
    {
        ...
        cerr << endl << "Usage: ./project path_to_euroc.yaml path_to_imu/data.csv path_to_cam0/data.csv path_to_cam0/data  strName" << endl;
        // ros::shutdown();
        return 1;
    }
    ...
    loadImageList(argv[3], iListData);
...
//为了使allimuData中的imu的第一帧和iListData中图像的第一帧对齐，去除了ListData中图像的第一帧时刻之前的imu数据
    ...
    delete [] fullPath;
    SLAM.SaveKeyFrameTrajectoryNavState(config._tmpFilePath +argv[5]+ "_norosMonoVio.txt");
